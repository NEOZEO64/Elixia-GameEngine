...
double Vector2D::len() const { return sqrt(pow(x, 2) + pow(y, 2)); }

double
Vector2D::ang() const { // to an vector (1,0) otherwise a vector facing east
...
Vector2D operator+(Vector2D const &thisVector) { return thisVector; }
